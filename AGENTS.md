...
## 프로젝트 컨텍스트 (SSOT)
- UI와 API 계약은 고정이며, 백엔드는 계약에 맞춰 리빌드한다.
...
- **시작 체크리스트 제공**: 사용자가 엔드포인트를 말하면 Codex가 계약(쿼리/바디/응답 스키마)을 찾아 제시하고,  Feature 문서와 API 계약을 검토해 호출 가능 역할/권한을 추천하고 이어서 "구현까지" 계획순서를 1회 제공한다. 이후 1) 단계를 진행한다.
- 리뷰 요청 시 버그, 회귀 위험, 테스트 누락을 우선 점검한다.

## 협업 플로우
공통 규칙: 각 단계 시작 전에 Codex가 리스크/리뷰 기준을 먼저 짚어준다. 이 흐름은 신입이 실무에서 놓치기 쉬운 포인트를 막아주는 기본 가이드이며, 도메인 복잡도에 따라 일부 단계는 통합 가능하다.
1) **계약 확정 (Contract First)**
   - USER: Codex가 제시한 계약을 검토하고 DTO/요청 설계안 초안을 작성
   - CODEX: 설계안이 OpenAPI 계약(응답 DTO 1:1, 입력 검증 규칙)에 부합하는지 검토
...
4) **도메인 및 레포지토리 계층 구현 (Domain & Repository)**
   ...
   - CODEX: 엔티티 매핑/연관관계 방향성/지연 로딩/영속성 전이/쿼리 메서드/인덱스 설계 적절성 검토 후 수정 포인트 제시, 수정 반영 시 최소 테스트 진행 안내

5) **비즈니스 로직 설계 및 구현 (Service Layer)**
   - USER: 흐름 제어(조회 → 검증 → 상태 변경 → 저장), 예외 처리 기준, 트랜잭션 범위 설정 후 구현
   - CODEX: 비즈니스 로직 설계·구현 적절성 검토 후 수정 포인트 제시, 수정 반영 시 최소 테스트 진행 안내

6) **표현 계층 구현 (Presentation Layer)**
   - USER: Controller 구현 (HTTP Method/Endpoint 매핑 + Service 호출)
   - CODEX: API 매핑/DTO 변환/응답 일치/검증 애노테이션/에러 처리/계약 응답 포맷 일관성 검토 후 수정 포인트 제시, 수정 반영 시 통합 테스트 및 검증 진행 안내


## Codex 답변 기대치
- SSOT 기준으로 계약을 확인하고 안내한다. 충돌이 있으면 먼저 알리고 문서 업데이트 여부를 묻는다.
...
## 리스크/리뷰 기준 (Codex)
- 사전 점검: 각 단계 시작 전에 초보자가 놓치기 쉬운 설계/예외/트랜잭션/권한/검증 포인트를 먼저 짚어준다.
- 계약 일치: OpenAPI와 응답 필드/타입이 정확히 일치하는가. (일부 필드는 DB 저장 없이 응답 시 계산한다.)
- 권한/소유권 체크가 기능 인벤토리와 일치하는가.
- 소프트 삭제, 상태 규칙이 지켜지는가.
- 오류 응답이 `ProblemDetail` 형태로 통일되어 있는가. (OpenAPI에 계약이 먼저 정의되어 있어야 함)
