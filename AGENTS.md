...
- **시작 체크리스트 제공**: 사용자가 엔드포인트를 말하면 Codex가 계약(쿼리/바디/응답 스키마)을 찾아 제시하고, 이어서 "구현까지" 계획순서를 1회 제공한 뒤 1) 단계를 진행한다.
...
1) **계약 확정 (Contract First)**
   ...
   - 체크리스트: 이 API의 호출 가능 역할/권한을 1단계에서 먼저 확정
...
4) **도메인 및 레포지토리 계층 구현 (Domain & Repository)**
   ...
   - CODEX: 엔티티 매핑/연관관계 방향성/지연 로딩/영속성 전이/쿼리 메서드/인덱스 설계 적절성 검토
...
6) **표현 계층 구현 (Presentation Layer)**
   ...
   - CODEX: API 매핑/DTO 변환/응답 일치/검증 애노테이션/에러 처리/계약 응답 포맷 일관성 검토

7) **검증 및 테스트 (Verification)**
   - USER: Postman/Swagger로 응답 JSON이 명세와 일치하는지 확인, 핵심 API 통합 테스트 작성(가능하면 수행). 
