...
| --- | --- | --- |
| `User` | Partial | 공통 로그인·세션 인증 주체와 조회 주체 식별 |
...
학생과 관리자는 공통 `User`를 로그인·세션 인증 주체로 사용한다. 거주 정보는
`Residence`, 관리자 여부는 계정 권한, 냉장고 담당 여부는 활성 업무 배정으로
조회한다. 학생·관리자 전용 필드가 확인되기 전에는 별도 Profile Entity를 만들지
않으며 API 프로필 응답을 DB Entity와 동일하게 보지 않는다. 구체 인증 자격증명과
계정 권한 매핑은 인증 수직 슬라이스에서 확정한다.

| Column | Constraint | Status |
| --- | --- | --- |
| `id` | PK, UUID | Existing skeleton |
| `enabled` | NOT NULL | 합의된 계정 로그인 가능 상태. 영구 퇴소 시 `false`; 인증 수직 슬라이스에서 매핑 확정 |

`enabled`는 거주 중 여부를 대신하지 않는다. 현재 거주 호실과 거주 이력은
`Residence`에서 확인한다. 영구 퇴소에서는 활성 거주 관계 종료와 계정 비활성화를
같은 트랜잭션으로 처리한다. 휴관은 둘 중 어느 상태도 바꾸지 않는다.
