...
`Scope`는 적용 시점이며 `Status`는 정책의 확정 여부다. `Scope=Post-MVP`인
`Confirmed` 정책은 보존하되 해당 확장 시작 전에는 구현하지 않는다. 기존
`Status=Post-MVP` 행은 후속 정책 후보를 뜻하며 확장 시 검토한다.
...
### 적용 범위 변경 기록 — 2026-09-14

1차 완료와 이후 확장 원칙은 [README](../../README.md#이후-확장-원칙)를 따른다.
검사·관리자·거주 이동·시설 기능의 후속 이동은 적용 시점 변경이며 기존
`Confirmed` 정책을 철회하지 않는다. 미결 정책도 이번 범위 합의로 자동 확정하지 않는다.
공통 인증·소유권 경계는 1차 API에도 적용하며 관리 UI나 배정 변경 기능을
선행 구현한다는 의미가 아니다. API별 범위·진행 상태는 API 구현 현황에서 관리한다.
...
| PRJ-002 | MVP | `Confirmed` | 프론트 필수 계약 연동의 수행 범위와 UI 변경 합의는 AGENTS.md 1장의 공통 협업 규칙을 따른다. | 사용자 합의 |
...
| INV-004 | Post-MVP | `Confirmed` | 검사로 잠긴 대상은 일반 사용자가 수정하거나 삭제할 수 없다. | Feature Inventory와 MVP 합의 |
...
| INV-022 | Post-MVP | `Review Required` | `displayName` 변경이 기존 실물 표기와 검사·감사 이력에 미치는 영향, 이름 정규화와 소프트 삭제 후 재사용 기준을 확정해야 한다. | 표시명 변경 운영 정책 미정 |
...
| INV-027 | Post-MVP | `Confirmed` | 방 이동으로 새 호실의 Slot 이용 범위가 적용되기 전에, 사용자가 이전 호실을 통해서만 접근할 수 있던 Slot의 기존 포장을 정리해야 한다. 구체적인 정리 확인·이동 처리와 실패 응답은 거주 이동 또는 Bundle 수직 슬라이스에서 계약한다. | 방 이동 직후 기존 물품이 접근 불가능해지는 문제를 방지하기 위한 2026-07-26 합의 |
| INV-028 | Post-MVP | `Review Required` | 냉장고 담당자의 관리 Slot과 관리자의 전체 Slot을 일반 거주자용 `GET /fridge/slots`에 함께 반환할지, 역할별 별도 조회 계약을 둘지는 후속 Task에서 확정한다. | 첫 Slot Task에서 다중 역할과 결과 합집합을 제외한 2026-07-27 범위 결정 |
...
| FAC-001 | Post-MVP | `Confirmed` | 다목적실과 도서관은 모든 활성 거주자가 이용할 수 있다. 관리자 등 비거주 계정의 일반 이용 권한은 별도로 정의한다. | 2026-07-26 사용자 확인 |
| LAUN-001 | Post-MVP | `Confirmed` | 일반 세탁실은 남성용과 여성용으로 구분하며, 활성 거주자는 자신의 성별에 허용된 세탁실과 그 안의 세탁기·건조기를 이용할 수 있다. 성별과 현재 거주 층 중 무엇을 권한 판정의 원천으로 저장할지는 세탁실 수직 슬라이스에서 결정한다. | 2026-07-26 사용자 확인 |
| LAUN-002 | Post-MVP | `Confirmed` | 공용 건조기는 성별 구분 없이 모든 활성 거주자가 이용할 수 있다. | 2026-07-26 사용자 확인 |
...
| AUTH-002 | MVP | `Confirmed` | MVP는 온라인 회원가입과 관리자 계정 발급 API를 제공하지 않는다. 로그인에 필요한 계정·호실·거주 관계는 개발환경 전용 seed로 준비한다. 운영환경의 최초 관리자와 사용자 계정 공급 절차는 배포 전 별도로 확정하며 개발 seed를 운영에 노출하지 않는다. | 원본의 가입 후 승인 흐름과 현재 비활성 회원가입 UI를 비교한 뒤, 첫 로그인·Slot 수직 슬라이스의 학습 범위를 작게 유지하기로 한 2026-09-16 사용자 합의 |
...
| AUTH-018 | MVP | `Confirmed` | 사용자 프로필의 현재 호실은 표시값 `roomNumber`와 구조화된 층 번호 `floorNo`를 별도 필드로 반환한다. 층 판정·필터·정렬을 위해 `roomNumber`의 앞자리를 파싱하지 않는다. | 2026-08-02 사용자 합의. 현재 `301 → 3층` 표기 관례와 도메인의 층 정보를 분리해 호실 번호 체계 변경에 결합되지 않게 한다. |
| AUTH-019 | MVP | `Confirmed` | 학생과 관리자는 공통 `User`를 로그인·세션 인증 주체로 사용한다. 거주 자격은 `Residence`, 관리자 여부는 계정 권한, 냉장고 담당 업무는 `SlotManagerAssignment`로 분리한다. 전용 필드가 확인되지 않은 학생·관리자 Profile Entity는 만들지 않는다. | 두 인증 저장소의 조회·중복·비밀번호 관리 경로를 통합하고 생명주기가 다른 업무 정보를 분리하기로 한 2026-09-16 사용자 합의 |
| AUTH-020 | Post-MVP | `Confirmed` | 휴관은 퇴소와 다르며 거주계약과 `Residence` 이력을 유지한다. 휴관 중 거주자 로그인은 차단하고 관리자는 시설 운영 작업을 위해 로그인할 수 있다. 휴관을 개별 User 비활성이나 Residence 종료로 표현하지 않는다. 휴관 전환 방식과 로그인 실패 계약은 후속 Task에서 확정한다. | 기숙사 운영에는 필요하지만 Release 1 범위에서는 제외하기로 한 2026-09-16 사용자 확인. 원본 Feature Inventory와 UI Flow에는 휴관 흐름이 명시되지 않음 |
| AUTH-021 | MVP | `Confirmed` | 영구 퇴소 시 활성 `Residence`를 종료하고 `User.enabled = false`를 같은 트랜잭션에서 적용하며 계정·거주·물품 이력은 보존한다. 비활성 계정의 로그인 실패는 `AUTH-014`·`AUTH-015`에 따라 비밀번호 일치 여부를 외부 응답에서 구분하고 세션을 생성하지 않는다. 영구 퇴소 처리 API의 구현 범위는 별도로 정한다. | 퇴소 후 재입소가 없는 운영 사실과, 사용자·행정실에 정확한 원인을 알리되 비밀번호 오류로 계정 상태를 노출하지 않기로 한 2026-09-16 사용자 합의 |
...
| AUTH-009 | Post-MVP | `Confirmed` | 관리자 UI는 층 전체와 개별 Slot 선택을 제공한다. 층 전체 선택은 선택 시점의 해당 층 활성 Slot에 관리 배정을 일괄 생성하는 편의 기능이며, 이후 추가되는 Slot을 자동 포함하지 않는다. | 백엔드 권한 단위를 Slot으로 통일한 MVP 결정 |
...
| ID | Scope | Status | Policy | Evidence / Notes |
| --- | --- | --- | --- | --- |
| INSP-001 | Post-MVP | `Confirmed` | 냉장고 담당자는 자신에게 활성 관리 배정된 Slot만 검사하고 처리할 수 있다. 층 전체 검사에서 필요한 관리 범위와 일부 Slot 담당자의 검사 흐름은 Phase 3에서 확정한다. | Feature Inventory의 층 담당 의도를 Slot 단위 관리 권한에 맞게 재설계 |
| INSP-002 | Post-MVP | `Confirmed` | 모든 검사 대상에 PASS, WARNING 또는 DISPOSE 조치가 기록되어야 제출할 수 있다. | Feature Inventory와 실제 프론트 제출 조건 |
| INSP-003 | Post-MVP | `Confirmed` | 제출된 검사 결과는 첫 검사 확장 범위에서 일반 수정할 수 없다. 정정 흐름은 별도 확장 정책으로 다룬다. | MVP 범위 합의 |
| INSP-004 | Post-MVP | `Confirmed` | 중복 검사 시작과 중복 제출이 동일 결과를 두 번 생성하지 않도록 서버가 보장한다. | MVP 범위와 동시성 핵심 시나리오 |
| INSP-005 | Post-MVP | `Review Required` | 검사 잠금의 정확한 대상, 만료 시점, 활동 시 연장 기준과 강제 종료 권한을 확정해야 한다. | Feature Inventory는 30분 잠금을 제시하나 상세 계약 미확정 |
...
| ID | Scope | Status | Policy | Evidence / Notes |
| --- | --- | --- | --- | --- |
| ADM-001 | Post-MVP | `Confirmed` | 관리자 API는 서버에서 관리자 권한을 검증한다. UI의 메뉴 숨김만으로 권한을 보장하지 않는다. | 보안 기본 원칙 |
| ADM-002 | Post-MVP | `Review Required` | 최소 관리자 기능별 조회 범위와 강제 삭제·상태 변경의 의미를 확정해야 한다. | README의 후속 관리자 범위에 포함되며 세부 정책 미확정 |
| ADM-003 | Post-MVP | `Confirmed` | 관리자는 전체 냉장고 데이터를 조회할 수 있지만 거주자 권한을 상속하지 않는다. 운영상 정정·강제 삭제는 일반 사용자 API와 구분하고 수행자와 사유를 기록한다. 관리자 대리 등록은 실제 요구가 생길 때 대상 거주자를 명시하는 별도 계약으로 검토한다. | 관리자와 데이터 소유자를 분리한 MVP 경계 |
...
- `CreateBundleRequest.items`는 필수 배열이지만 `minItems`가 없어 `INV-006`의
  최소 1개 조건을 표현하지 않는다. Bundle 생성 Task에서 계약을 보완해야 한다.
- `POST /fridge/bundles`의 용량 초과 `422`는 현재 `CreateBundleResponse`와
  `*/*`로 선언되어 공통 ProblemDetail 결정과 맞지 않는다. 관련 관리자 오류에도
  성공 DTO 참조가 남아 있다. 각 Task에서 오류 계약을 합의한 뒤 갱신한다.
- 프론트 인증 전환의 실제 적용 상태와 남은 CSRF 연동은 Frontend Integration
  Baseline에서 관리한다. 토큰 인증은 `AUTH-011`의 Post-MVP 계약이 확정되기 전
  재사용하지 않는다.
