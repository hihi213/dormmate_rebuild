...
- **인증 선택:** Spring Security 서버 세션과 `HttpOnly`, `SameSite=Lax` 쿠키를 사용한다. 배포 HTTPS 환경에서는 `Secure`를 적용한다. MVP는 단일 백엔드 인스턴스의 메모리 세션을 사용하며 Access/Refresh Token, `deviceId`, Redis Session은 도입하지 않는다.
...
- **재검토 조건:** 다중 인스턴스 배포나 세션 공유 요구가 생기면 Spring Session과 Redis를 비교한다.
...
- **조회 범위:** 일반 사용자의 결과는 거주자로 현재 배정된 Slot과 냉장고 담당자로 관리 배정된 Slot의 합집합이며 중복 Slot은 한 번만 반환한다. 관리자는 전체 Slot을 조회한다.
