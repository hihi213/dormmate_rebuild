...
- **변동 권한:** 거주 자격, 층별장 여부와 관리 Slot은 세션이나 `CurrentActor`에 캐시하지 않고 DB의 현재 활성 배정을 조회한다. 층별장은 거주자 권한을 유지하며 관리 범위는 `FridgeSlotManagerAssignment`로 표현한다. 관리자처럼 계정 자체에 부여되는 권한만 `accountAuthorities`에 둔다.
...
- **최종 선택:** 층별장은 활성 거주자에게 추가하는 기간성 업무로 보고, 관리 권한은 `FridgeSlotManagerAssignment(userId, slotId, assignedAt, releasedAt, assignedBy)`를 통해 Slot 단위로 저장한다. 층별장은 거주자 권한을 유지한다.
...
- **조회 범위:** 일반 사용자의 결과는 거주자로 현재 배정된 Slot과 층별장으로 관리 배정된 Slot의 합집합이며 중복 Slot은 한 번만 반환한다. 관리자는 전체 Slot을 조회한다.
